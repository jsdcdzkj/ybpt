...
import com.jsdc.ybpt.model.OrgReconciliationMonth;
...
@Service
public class OrgReconciliationMonthService extends BaseService<OrgReconciliationMonth> {
    @Autowired
    ...
    public ResultInfo checkReconciliation(OrgReconciliationMonth o) {
        //查询本月如果有对账成功数据，则不新增
        OrgReconciliationMonth oo = orgReconciliationMonthService.getById(o.getId());
        if(oo != null && "1".equals(oo.getCheck_status())){
            return ResultInfo.error("该条对账已经成功，无需重复对账！");
        }
        //查询对账记录医保端数据
        BizReconciliationMonth b = bizReconciliationMonthService.getById(o.getBizId());
        //对账
        if (
                ...
                contrast(b.getHifob_pay(), o.getHifob_pay()) &&
     ...
     * @return
     */
    public ResultInfo month_reconciliation(ReconciliationApiVo data){
        List<OrgReconciliationMonth> resultList = new ArrayList<>();
        //查询日对账数据
        QueryWrapper qw = new QueryWrapper();
        qw.eq("fixmedins_code",data.getFixmedins_code());
        qw.eq("settle_date",data.getSettle_date());
        qw.eq("is_del","0");
        List<OrgReconciliationMonth> orgReconciliations = this.list(qw);
        HashMap<String,OrgReconciliationMonth> orgMap = new HashMap();
        for (OrgReconciliationMonth o: orgReconciliations) {
            //对账类型+统筹区确认唯一记录
            orgMap.put(o.getReconciliation_type()+"_"+o.getInsu_admdvs(),o);
        }
        //遍历数据验证结果
        for (OrgReconciliation o: data.getMonth_details()) {
            //查询是否有此对账信息
            if(orgMap.containsKey(o.getReconciliation_type()+"_"+o.getInsu_admdvs())){
                OrgReconciliationMonth sys_o = orgMap.get(o.getReconciliation_type()+"_"+o.getInsu_admdvs());
                //如果已经对账成功，则直接返回结果
                if("1".equals(sys_o.getCheck_status())){
                    ...
                    if(r.getCode() == 0){
                        OrgReconciliationMonth o_r = (OrgReconciliationMonth) r.getData();
                        resultList.add(o_r);
                    }else{
