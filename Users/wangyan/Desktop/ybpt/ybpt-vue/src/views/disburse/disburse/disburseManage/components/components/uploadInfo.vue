...
export default {
  ...
  methods: {
    showDia(row, uploadData) {
      ...
      this.selectDataAll = row.convertDataDetail
      this.selectDataTotal = row.dataDetail.length ? row.dataDetail.length : 0
      this.fetchData()
      // console.log(row,'row');
    ...
    fetchData() {
      ...
      if (this.activeName == 'first') {
        //汇总数据
        that.zgSummaries = this.rowData.convertZgSummaries
        that.jmSummaries = this.rowData.convertJmSummaries
      } else {
