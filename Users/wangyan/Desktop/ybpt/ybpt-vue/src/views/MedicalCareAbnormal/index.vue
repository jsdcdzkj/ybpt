      ...
      <vab-query-form-right-panel :span="24">
        ...
        >
          <el-form-item label=" " prop="upload_no">
            <el-select
                v-model.trim="queryForm.upload_no"
                clearable
                placeholder="请选择批次"
                class="w"
            >
          ...
          <el-form-item prop="if_upload">
            <el-select
                v-model.trim="queryForm.if_upload"
                clearable
                placeholder="请选择是否修改"
                class="w"
            >
...
import {
  ...
  getMedicalCareAbnormalList,
  getUploadNo,
  importData,
  importData2,
} from '@/api/MedicalCareAbnormal'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        upload_no: [
          {required: true, trigger: 'blur', message: '请选择批次'},
        ],
      },
    }
  },
  ...
  methods: {
    ...
    handleExport(row) {
      ...
      this.$baseConfirm('你确定要导出当前信息吗', null, async () => {
        ...
        await exportSettleAbnormalData(this.queryForm).then((res) => {
          let fileName = '徐州医疗人员异常数据导出.xlsx'
    ...
    async fetchData() {
      ...
      this.getArea()
      this.listLoading = true
      const res = await getMedicalCareAbnormalList(this.queryForm)
      if ((res.code = '0')) {
