      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        <el-card class="card" shadow="never">
          ...
          >
            ...
            <el-table-column
              show-overflow-tooltip
              prop="startDateOfMedicalTreatment"
              label="就医开始日期"
              align="center"
              width="200px"
            ></el-table-column>
            <el-table-column
              show-overflow-tooltip
              prop="endDateOfMedicalTreatment"
              align="center"
              label="就医结束日期"
            ></el-table-column>
            <el-table-column
...
import { reimbursementSettlementData,reimbursementSettlementDataExport } from '@/api/query.js'
export default {
  name: 'Index',
  ...
  methods: {
    ...
    moreQuery() {
     ...
     fetchData() {
      var that = this ;
       that.loading = true ;
       reimbursementSettlementData(that.queryForm).then((res) => {
        if(res.code == 0){
    ...
    async handleExport() {
      this.$baseConfirm('确认导出吗？', null, async () => {
        ...
        const res = await reimbursementSettlementDataExport(this.queryForm);
        let fileName = "零星报销结算进度.xls";
