  ...
  <div class="main-container" >
    <el-row :gutter="20">
            ...
            <el-row :gutter="20">
              <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8">
                <el-form-item label="统筹区" >
                  <el-select v-model.trim="queryForm.insu_admdvs" class="w">
              ...
              <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8">
                <el-form-item label="人员参保状态">
                  <el-select v-model="queryForm.psn_insu_stas" style="width: 100%">
                    <el-option v-for="item in psnInsuStasData" :key="item.nat_dic_val_code" :label="item.nat_dic_val_name"
              ...
              <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8">
                <el-form-item label="人员类别">
                  <el-select v-model="queryForm.psn_type" style="width: 100%">
                    <el-option v-for="item in psnTypeData" :key="item.nat_dic_val_code" :label="item.nat_dic_val_name"
                               :value="item.nat_dic_val_code">
                    </el-option>
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        <el-card class="card" shadow="never">
          <div slot="header">
            <span class="tips">城乡居民基本医疗保险人员统计</span>
...
import { insuranceData,insuranceDataExport } from '@/api/query.js'
export default {
  name: 'Index',
  ...
  methods: {
    ...
    moreQuery() {
     ...
     fetchData() {
      var that = this ;
       that.loading = true ;
       insuranceData(that.queryForm).then((res) => {
        if(res.code == 0){
    ...
    async handleExport() {
      this.$baseConfirm('确认导出吗？', null, async () => {
        ...
        const res = await insuranceDataExport(this.queryForm);
        let fileName = "城乡居民基本医疗保险人员统计.xls";
    ...
    async getPsnInsuStas() {//PSN_INSU_STAS
      upData('PSN_INSU_STAS').then((res) => {
        if(res.code == 0){
          this.psnInsuStasData = res.data ;
        }
      })
    },
    async getPsnType() {//人员类别
      upData('PSN_TYPE').then((res) => {
        if(res.code == 0){
          this.psnTypeData = res.data ;
        }
      })
    },
