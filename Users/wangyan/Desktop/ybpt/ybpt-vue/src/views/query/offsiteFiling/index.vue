...
import { offsiteFilingData,offsiteFilingDataExport } from '@/api/query.js'
export default {
  name: 'Index',
  ...
  methods: {
    ...
    moreQuery() {
     ...
     fetchData() {
      var that = this ;
       that.loading = true ;
       offsiteFilingData(that.queryForm).then((res) => {
        if(res.code == 0){
    ...
    async handleExport() {
      this.$baseConfirm('确认导出吗？', null, async () => {
        ...
        const res = await offsiteFilingDataExport(this.queryForm);
        let fileName = "零报复核数据.xls";
