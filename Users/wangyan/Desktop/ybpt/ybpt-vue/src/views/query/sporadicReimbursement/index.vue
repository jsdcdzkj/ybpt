...
import { sporadicReimbursementData,sporadicReimbursementDataExport } from '@/api/query.js'
export default {
  name: 'Index',
  ...
  methods: {
    ...
    moreQuery() {
     ...
     fetchData() {
      var that = this ;
       that.loading = true ;
       sporadicReimbursementData(that.queryForm).then((res) => {
        if(res.code == 0){
    ...
    async handleExport() {
      this.$baseConfirm('确认导出吗？', null, async () => {
        ...
        const res = await sporadicReimbursementDataExport(this.queryForm);
        let fileName = "零星报销受理情况.xls";
