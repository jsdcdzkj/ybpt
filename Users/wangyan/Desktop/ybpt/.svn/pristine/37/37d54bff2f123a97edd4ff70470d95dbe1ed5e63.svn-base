<template>
    <el-drawer :title="title" :visible.sync="dialog" direction="rtl" :with-header="false" :before-close="handleClose"
        custom-class="box_drawer" size="1060px" ref="drawer" :close-on-click-modal="true" append-to-body>
        <!-- pdf 内嵌 -->
        ...
        <div class="drawer_content">
            <div class="drawer_footer">
                ...
                <el-button type="primary" @click="download" v-if="showDown">下载附件</el-button>
                <el-button type="primary" @click="downPdf">下载PDF</el-button>
                <el-button @click="cancelForm">关 闭</el-button>
            </div>
        </div>
        <el-dialog title="驳回理由" :visible.sync="remarkVisible" append-to-body width="700px">
            <el-form ref="form" :model="form" label-width="100px">
                <el-row :gutter="20">
                    <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
                        <el-form-item label="说明" prop="regeditType">
                            <el-input type="textarea" v-model="form.approval_opinion"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
export default {
    ...
    methods: {
        ...
        async details(row, type) {
            ...
            if ("1" == type) {//查看详情
                that.PDFsrc = row.pdfPath
                that.downPath = row.downPdfPath

            } else if ("2" == type) {//查看受理书
                //申报类型 1.西药 2.中成药 3.中药饮片 4.医疗服务 5. 耗材 6.自设项目
                if (row.type != '1' && row.type != '2' && row.type != '3') {
                    that.showExcel = true
                }
                that.showDown = false
                that.PDFsrc = row.acceptLetterPdfPath
                that.downPath = row.acceptLetterDownPdfPath
            }
