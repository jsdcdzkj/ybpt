      ...
      <el-row :gutter="20">
        ...
        <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
          <el-form-item label="适用范围" prop="applicable_scope">
            <el-select
              v-model.trim="form.applicable_scope"
              @change="applicable_scope"
              clearable
              class="w"
            >
              <el-option
...
<script>
  ...
  import { getListAll, getListUnion } from '@/api_check/tijianguanli'
  import { edit, toEdit } from '@/api_check/taocan'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          pack_year: [
            { required: true, trigger: 'blur', message: '请选择年份' },
          ],
          pack_name: [
            { required: true, trigger: 'blur', message: '请输入名称' },
          ],
          applicable_scope: [
            { required: true, trigger: 'blur', message: '请选择适用范围' },
          ],
          pack_money: [
            { required: true, trigger: 'blur', message: '请输入金额' },
          ],
          item_to_meal: [
            { required: true, trigger: 'blur', message: '请选择体检项目' },
          ],
        },
        title: '',
    ...
    methods: {
      ...
      async getListAll() {
        ...
        const { data } = await getListAll({ year: this.form.pack_year })
        this.medical_item = []
      ...
      async getMedicalItem() {
        ...
        const { data } = await getListAll({ year: this.year })
        this.medical_item = []
        if (data) {
          for (let i = 0; i < data.length; i++) {
            this.medical_item.push({
