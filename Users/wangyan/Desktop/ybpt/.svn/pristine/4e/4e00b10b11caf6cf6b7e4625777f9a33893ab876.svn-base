...
public class FddService {
     ...
     * @param content
     */
    public String customSeal(String customerId, String content, String destPath, String fileName){
        FddBaseClient baseClient = new FddBaseClient(appId, appKey, v, host);
        ...
        if(res.getString("code").equals("1")){
            ...
            FileUtils.base64ToFile(destPath, base64, fileName);
     ...
     * @return
     */
    public String autoSignAuthorize(String customerId){
        FddBaseClient client = new FddBaseClient(appId, appKey, v, host);
        ...
        params.setCustomerId(customerId);
        //同步通知签署结果地址
        params.setReturnUrl("1");
        //异步通知签署结果地址
        params.setNotifyUrl("1");
...
//        FddBaseClient client = new FddBaseClient("406848", "0mAuvC0y8DWQMs0QsHQmTK7B", "2.0", "http://test.api.fabigbig.com:8888/api/");
//        ExtSignParams params = new ExtSignParams();
//        String transactionId = UUID.randomUUID().toString().replaceAll("-", "");
//        params.setTransactionId(transactionId);//平台自定义唯一交易号
//        params.setContractId("0d0f01fcf7094764b34e367271d9d34f");//此处传入调用上传或填充合同接口成功时定义 的合同编号
//        params.setCustomerId("FAB958302C85B0B702E6D9D0C75AE5D6");//此处传入认证成功后成功申请证书的客户编号
//        params.setDocTitle("测试签章");
