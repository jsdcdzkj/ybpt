<template>
  <el-drawer title="预览数据" :visible.sync="dialog" direction="rtl" custom-class="box_drawer" size="80%" ref="drawer"
    append-to-body>
    <div class="drawer_content">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="汇总数据" name="first">
          <div v-loading="loading" element-loading-text="下载中。。。" element-loading-spinner="el-icon-loading"
            ...
            <el-table :data="zgSummaries" border stripe class="w" v-loading="listLoading"
              :element-loading-text="elementLoadingText" height="32vh">
        ...
        <el-tab-pane label="数据明细" name="second">
          <div class="drawer_main" v-loading="loading" element-loading-text="下载中。。。"
            ...
            <el-table :data="list" border stripe class="w" v-loading="listLoading"
              ...
              <el-table-column show-overflow-tooltip prop="orgName" label="医疗机构名称" width="200px" align="center"
                fixed="left"></el-table-column>
              <el-table-column show-overflow-tooltip prop="jb" label="级别" width="80px" align="center"
                fixed="left"></el-table-column>
              <el-table-column label="职工医保（万元）" align="center">
                <el-table-column :label="table_title_add" align="center" :class-name="'add-table-title'">
                  ...
                  <el-table-column show-overflow-tooltip prop="zgLjGzjjfsje" label="个账基金发生金额" width="160px"
                    align="center" :class-name="'add-table-title'"></el-table-column>
                ...
                <el-table-column :label="table_title_current" align="center" :class-name="'current-table-title'">
                  ...
                  <el-table-column show-overflow-tooltip prop="zgDyGzjjfsje" label="个账基金发生金额" width="160px"
                    align="center" :class-name="'current-table-title'"></el-table-column>
                  ...
                  <el-table-column show-overflow-tooltip prop="zgDyJjbfje" label="基金拨付金额" width="160px" align="center"
                    :class-name="'current-table-title'"></el-table-column>
                  <el-table-column show-overflow-tooltip prop="zgDyKhbzj" label="考核保证金累计" width="160px" align="center"
                    :class-name="'current-table-title'"></el-table-column>
                  <el-table-column show-overflow-tooltip prop="zgDyKk" label="累计扣款" width="160px" align="center"
                    :class-name="'current-table-title'"></el-table-column>
                </el-table-column>
              </el-table-column>
              ...
              <el-table-column label="居民医保（万元）" align="center" :class-name="'current-table-title-parent'">
                ...
                <el-table-column :label="table_title_current" align="center" :class-name="'current-table-title'">
                  ...
                  <el-table-column show-overflow-tooltip prop="jmDyDbsbbfje" label="大病商保拨付金额" width="160px"
                    align="center" :class-name="'current-table-title'"></el-table-column>
                  <el-table-column show-overflow-tooltip prop="jmDyKhbzj" label="考核保证金累计" width="160px" align="center"
                    :class-name="'current-table-title'"></el-table-column>
                  <el-table-column show-overflow-tooltip prop="jmDyKk" label="累计扣款" width="160px" align="center"
                    :class-name="'current-table-title'"></el-table-column>
                </el-table-column>
              </el-table-column>
            </el-table>
            <el-pagination background @size-change="selectSizePage" @current-change="selectCurrentPage"
              :current-page="watchPage.pageNum" :page-sizes="[10, 20, 30, 40]" :page-size="watchPage.pageSize"
              layout="total, sizes, prev, pager, next, jumper" :total="selectDataTotal"></el-pagination>
      ...
      <div class="drawer_footer">
        ...
        <el-button type="primary" @click="handleDownLoad">确定生成</el-button>
      </div>
    </div>
  </el-drawer>
</template>
...
<script>
import {
  approNoticeViewSummary,
  approNoticeViewDataDetail,
  approNoticeDownloadSummary,
  approNoticeDownloadDataDetail,
  appropDocumentGenerate,
...
export default {
  ...
  methods: {
    showDia(row, uploadData) {
      ...
      // this.table_title_current = row.year + '年' + row.month + '月当月'
      this.rowData = row
      this.uploadForm = uploadData
      this.selectDataAll = row.convertDataDetail ? row.convertDataDetail : []
      this.selectDataTotal = row.dataDetail && row.dataDetail.length ? row.dataDetail.length : 0
      this.fetchData()
      // console.log(row,'row');
    ...
    fetchData() {
      ...
      if (this.activeName == 'first') {
        //汇总数据
        that.zgSummaries = this.rowData.convertZgSummaries ? this.rowData.convertZgSummaries : []
        that.jmSummaries = this.rowData.convertJmSummaries ? this.rowData.convertJmSummaries : []
      } else {
        this.watchPage.pageNum = 1
        this.selectDataPageChange()
      }
    },
    // 自定义分页
    selectDataPageChange(val) {
      let pageNum = this.watchPage.pageNum
      let pageSize = this.watchPage.pageSize
      let allData = JSON.parse(JSON.stringify(this.selectDataAll))
      this.list = allData.slice(pageSize * (pageNum - 1), pageSize * pageNum);
      console.log(this.list, 'this.list');
    },
    selectSizePage(val) {
      this.watchPage.pageNum = 1
      this.watchPage.pageSize = val
      this.selectDataPageChange()
    },
    selectCurrentPage(val) {
      this.watchPage.pageNum = val
      this.selectDataPageChange()
    },
    ...
    handleDownLoad() {
      appropDocumentGenerate(this.rowData).then((res) => {
        if (res.code == 0) {
          this.dialog = false
          this.$emit('closes')
          this.$message({
