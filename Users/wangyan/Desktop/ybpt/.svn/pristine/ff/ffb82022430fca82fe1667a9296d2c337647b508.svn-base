    ...
    >
        ...
        <div class="drawer_content">
            <div class="drawer_footer">
                ...
                <el-button type="primary" v-if="showPdf" @click="downPdf">下载PDF</el-button>
...
import {signConfirmation, viewPdf} from "@/api/liquidation";
...
export default {
        ...
        methods: {
            ...
            async details(row) {
              ...
              if(row.status == 3){
                  console.info(row);
                that.filePath = row.centre_pdf_path
                that.PDFsrc = row.centre_pdf_path
                ...
                await setTimeout(function () {
                  ...
                  viewPdf({id: row.id}).then((res) => {
                    loading.close();
                    if (res.code == 0) {
                      that.filePath = res.data.fileUrl
                      that.PDFsrc = fileURL + res.data.fileUrl + '?n=' + res.data.fileName + '&download=0'
