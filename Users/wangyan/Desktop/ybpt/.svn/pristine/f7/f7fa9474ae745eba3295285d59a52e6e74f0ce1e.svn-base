            ...
            <el-row :gutter="20">
              ...
              <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8">
                <el-form-item label="药品通用名">
                  <el-input
                          v-model.trim="queryForm.drug_genname"
                          placeholder="药品通用名"
                          @keyup.enter.native="queryData"
                  />
                </el-form-item>
              ...
              <el-col :xs="24" :sm="24" :md="8" :lg="8" :xl="8">
                <el-form-item label="药品商品名">
                  <el-input
                          v-model.trim="queryForm.drug_prodname"
                          placeholder="药品商品名"
                          @keyup.enter.native="queryData"
                  />
                </el-form-item>
...
import { medicineInfoData,medicineInfoDataExport } from '@/api/query.js'
export default {
  name: 'Index',
  ...
  methods: {
    ...
    moreQuery() {
     ...
     fetchData() {
      var that = this ;
       that.loading = true ;
       medicineInfoData(that.queryForm).then((res) => {
        if(res.code == 0){
    ...
    async handleExport() {
      this.$baseConfirm('确认导出吗？', null, async () => {
        ...
        const res = await medicineInfoDataExport(this.queryForm);
        let fileName = "药品目录信息查询.xls";
