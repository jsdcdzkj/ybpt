            ...
            <el-row :gutter="20">
              ...
              <el-col :xs="24" :sm="24" :md="6" :lg="6" :xl="6">
                <el-form-item label="批次名称">
                  <el-input
                    placeholder="请输入"
                    v-model.trim="queryForm.batch_name"
                    @keyup.enter.native="queryData"
                  />
                </el-form-item>
              </el-col>
      ...
      <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
        <el-card class="card table_card" shadow="never">
          ...
          >
            ...
            <el-table-column
              show-overflow-tooltip
              label="低于序时进度且非中选超量的产品数"
              width="160"
              prop="unselected_excess_number"
              align="center"
            ></el-table-column>
            <!-- upload_no 批次号 -->
            <el-table-column
    ...
    methods: {
      ...
      downloadModel() {
        templateDownload(this.queryForm).then((res) => {
          let fileName = '国采地区带量执行报表数据模版.xlsx'
      ...
      handleExport(row) {
        this.$baseConfirm('你确定要导出当前信息吗', null, async () => {
          ...
          await down(this.queryForm).then((res) => {
            let fileName = '国采地区带量执行报表导出.xlsx'
            let objectUrl = URL.createObjectURL(new Blob([res.data]))
            const link = document.createElement('a')
            link.download = decodeURI(fileName)
            link.href = objectUrl
