<template>
  ...
  >
    <el-form ref="form" :model="form" :rules="rules" label-width="120px">
      ...
      <el-form-item label="儿童限价" prop="child_price">
        <el-radio-group
          v-model="form.child_price"
          @input="handleChildPrice(form.child_price)"
        >
      ...
      <el-form-item label="价格" prop="price">
        <el-input
          ...
          autocomplete="off"
          min="0"
          @input="handlePrice(form.price)"
        ></el-input>
      </el-form-item>
      <el-form-item label="申报类型" prop="declare_type">
        <!--        <el-select v-model="form.declare_type" style="width: 100%">-->
        <!--          <el-option label="单人间床位费" value="单人间床位费"></el-option>-->
        <!--          <el-option label="部分医疗服务项目市场调节价" value="部分医疗服务项目市场调节价"></el-option>-->
        <!--          <el-option label="新增医疗服务项目试行价" value="新增医疗服务项目试行价"></el-option>-->
        <!--        </el-select>-->
        <el-input
          v-model="form.declare_type"
          placeholder="请输入"
          autocomplete="off"
          readonly
        ></el-input>
      ...
      <el-form-item label="项目执行科室" prop="departments">
        <el-input
  ...
  export default {
    ...
    data() {
      return {
        options: [
          ...
          {
            label: '13 社区卫生及预防保健项目',
            value: '13 社区卫生及预防保健项目',
          },
        ...
        rules: {
          declare_type: [
            { required: true, trigger: 'blur', message: '请选择申报类型' },
          ],
          price: [{ required: true, trigger: 'blur', message: '请输入价格' }],
          child_price: [
            { required: true, trigger: 'blur', message: '请确认儿童限价' },
          ],
          cost_type: [
            { required: true, trigger: 'blur', message: '请选择费用类型' },
          ],
        },
        title: '',
    ...
    methods: {
      showEdit(row) {
        ...
        this.form.medical_type = row.medical_type
        switch (row.medical_type) {
          case 4:
            this.form.declare_type = '市场调节价项目'
            break
          case 6:
            this.form.declare_type = '自设项目'
            break
          case 7:
            this.form.declare_type = '新增医疗服务项目'
            break
          case 8:
            this.form.declare_type = '市管未定价项目'
            break
          case 9:
            this.form.declare_type = '其他病房床位'
            break
          case 10:
            this.form.declare_type = '单人间、套房床位'
            break
        }
      ...
      handlePrice() {
        if (
          this.form.org_price == undefined ||
          this.form.org_price == null ||
          this.form.org_price == ''
        ) {
      ...
      handleChildPrice(childPrice) {
        if (childPrice == 0) {
          if (
            this.form.aggrement_lv == '1' ||
            this.form.aggrement_lv == '9' ||
            this.form.aggrement_lv == ''
          ) {
      ...
      fetchData(row) {
        verifySbZlProject({
          project_code: row.provincialProjectCode,
          type: this.form.medical_type,
        }).then((res) => {
          if (res.code == 0) {
            if (res.data != '') {
              ...
              if (
                this.form.child_price == '0' ||
                this.form.child_price == '1'
              ) {
