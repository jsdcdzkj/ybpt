    ...
    methods: {
      showDia(row) {
        ...
        if (row.manageLossOtherFee) {
          ...
          this.tableData4[0].aaa2 = row.manageLossOtherFee.managFee;
          this.tableData4[1].aaa2 = row.manageLossOtherFee.lossOtherFee;
          this.tableData4[2].aaa2 = row.manageLossOtherFee.subtotal;
        } else {
      ...
      // 提交修改
      submit(index, row) {
        if (this.form.materialConsumeList[index].amount == '') {
          this.$baseMessage('请填写数据信息', 'error')
          return
        }
        // 发送请求，隐藏输入框
      ...
      cancelUpdateG(index) {
        this.$confirm('取消修改？', '提示', {
          ...
          .then(() => {
            if (this.form.fixedAssetsDepreList[index].name != '小计') {
              this.form.fixedAssetsDepreList[index].name = ''
            }
            this.form.fixedAssetsDepreList[index].price = ''
            this.form.fixedAssetsDepreList[index].year = ''
            this.form.fixedAssetsDepreList[index].hour = ''
            this.form.fixedAssetsDepreList[index].amount = ''
      ...
      // 提交修改
      submitG(index, row) {
        if (this.form.fixedAssetsDepreList[index].amount == '') {
          this.$baseMessage('请填写数据信息', 'error')
          return
        }
        // 发送请求，隐藏输入框
