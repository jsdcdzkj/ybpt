...
import { maternityBenefitsData,maternityBenefitsDataExport } from '@/api/query.js'
export default {
  name: 'Index',
  ...
  methods: {
    ...
    moreQuery() {
     ...
     fetchData() {
      var that = this ;
       that.loading = true ;
       maternityBenefitsData(that.queryForm).then((res) => {
        if(res.code == 0){
    ...
    async handleExport() {
      this.$baseConfirm('确认导出吗？', null, async () => {
        ...
        const res = await maternityBenefitsDataExport(this.queryForm);
        let fileName = "生育津贴受理情况.xls";
