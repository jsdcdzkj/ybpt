...
import { personalIncomeAndExpenditure,personalIncomeAndExpenditureExport } from '@/api/query.js'
export default {
  name: 'Index',
  ...
  methods: {
    ...
    moreQuery() {
     ...
     fetchData() {
      var that = this ;
       that.loading = true ;
       personalIncomeAndExpenditure(that.queryForm).then((res) => {
        if(res.code == 0){
    ...
    async handleExport() {
      this.$baseConfirm('确认导出吗？', null, async () => {
        ...
        const res = await personalIncomeAndExpenditureExport(this.queryForm);
        let fileName = "个人账户收支情况.xls";
