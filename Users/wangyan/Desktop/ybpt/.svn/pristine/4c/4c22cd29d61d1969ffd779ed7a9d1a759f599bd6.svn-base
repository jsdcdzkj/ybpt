...
public class EvalFormulaConfigService extends BaseService<EvalFormulaConfig> {
    ...
    public ResultInfo genEarnestMoney_staff(EvalTaskManage taskManage,List<EvalOrgTask> orgTasks,List<EvalFormulaConfig> configList){
        BigDecimal passScore = new BigDecimal("0.6");
        //定义需要进行计算的中间变量
        ...
        for (EvalOrgTask orgTask : orgTasks) {
            ...
            //如果百分比得分小于passScore，则不参与考核
            if(new BigDecimal(orgTask.getScore()).divide(new BigDecimal(orgTask.getTotalScore())).compareTo(passScore) < 0){
                //扣除所有保证金
                orgTask.setRefundStaff("0");
                orgTask.setStaffRewards("-"+orgTask.getMarginStaff());
                orgTask.updateById();
                continue;
            }
            //机构指数
            ...
            if(!formula.contains("M")){
                ...
                orgTask.setStaffRewards(new BigDecimal(orgTask.getRefundStaff()).subtract(G).toString());
                //减去已返还保证金
                M = M.subtract(new BigDecimal(orgTask.getRefundStaff()));
                orgTask.updateById();
            }else{
                //高绩效
        ...
        if(overOrgs != null && overOrgs.size() > 0){
            for (int i = 0; i < overOrgs.size()-1; i++) {
                ...
                overOrg.setStaffRewards(new BigDecimal(overOrg.getRefundStaff()).subtract(G).toString());
                overOrg.updateById();
                //剩下的钱
...
//        }
    ...
    public ResultInfo genEarnestMoney_resident(EvalTaskManage taskManage,List<EvalOrgTask> orgTasks,List<EvalFormulaConfig> configList){
        ...
        for (EvalOrgTask orgTask : orgTasks) {
            ...
            if(new BigDecimal(orgTask.getScore()).divide(new BigDecimal(orgTask.getTotalScore())).compareTo(passScore) < 0){
                //扣除所有保证金
                orgTask.setRefundResident("0");
                orgTask.setResidentRewards("-"+orgTask.getMarginResident());
                orgTask.updateById();
                continue;
            }
            //机构指数
            ...
            if(!formula.contains("M")){
                ...
                orgTask.setResidentRewards(new BigDecimal(orgTask.getRefundResident()).subtract(G).toString());
                //减去已返还保证金
                M = M.subtract(new BigDecimal(orgTask.getRefundResident()));
                orgTask.updateById();
            }else{
                //高绩效
                overOrgs.add(orgTask);
        ...
        if(overOrgs != null && overOrgs.size() > 0){
            for (int i = 0; i < overOrgs.size()-1; i++) {
                ...
                overOrg.setResidentRewards(new BigDecimal(overOrg.getRefundResident()).subtract(G).toString());
                overOrg.updateById();
                //剩下的钱
    ...
    public ResultInfo genEarnestMoney_medical(EvalTaskManage taskManage,List<EvalOrgTask> orgTasks,List<EvalFormulaConfig> configList){
        ...
        for (EvalOrgTask orgTask : orgTasks) {
            ...
            if(new BigDecimal(orgTask.getScore()).divide(new BigDecimal(orgTask.getTotalScore())).compareTo(passScore) < 0){
                //扣除所有保证金
                orgTask.setRefundMedical("0");
                orgTask.setMedRewards("-"+orgTask.getMarginMedical());
                orgTask.updateById();
                continue;
            }
            //机构指数
            ...
            if(!formula.contains("M")){
                ...
                orgTask.setMedRewards(new BigDecimal(orgTask.getRefundMedical()).subtract(G).toString());
                //减去已返还保证金
                M = M.subtract(new BigDecimal(orgTask.getRefundMedical()));
                orgTask.updateById();
            }else{
                //高绩效
                overOrgs.add(orgTask);
        ...
        if(overOrgs != null && overOrgs.size() > 0){
            for (int i = 0; i < overOrgs.size()-1; i++) {
                ...
                overOrg.setMedRewards(new BigDecimal(overOrg.getRefundMedical()).subtract(G).toString());
                overOrg.updateById();
                //剩下的钱
