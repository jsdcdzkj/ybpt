...
//制剂目录导出全部
export function catalogExport(data) {
  return request({
    url: '/catalog/exportData',
    method: 'post',
    responseType: 'blob',
    data,
  })
...
//导入数据
let urlUpload3 = baseURL + 'catalog/importData'
export async function pcatalogimportData(form) {
  var result = {}
  if (
    null != form.get('file') &&
    '' != form.get('file') &&
