    ...
    >
      ...
      <el-table-column label="选择" width="80" align="center">
        <template slot-scope="scope">
          <el-radio :label="scope.row.medins_code" v-model="id"><span></span></el-radio>
        </template>
      </el-table-column>
      <el-table-column
        label="序号"
...
<script>
import { selectMedinsInfoType } from '@/api/medinsInfoB.js'
export default {
  name: 'Index',
  components: {},
  data() {
  ...
  methods: {
    ...
    async fetchData() {
      var that = this ;
      // if(that.queryForm.medins_code =="" && that.queryForm.medins_name ==""){
      //   that.$baseMessage('请输入编号或名称', 'error')
      // }else {
        selectMedinsInfoType(that.queryForm).then((res) => {
    ...
    showDia(row) {
      this.list =[] ;
      this.total = 0 ;
      this.queryForm.fixmedins_type = row ;
